...
def bfile_from_ops(ops=None, ops_path=None, filename=None, device=None):
    ...
    bfile = BinaryFiltered(
        ...
        channel_groups=ops.get('channel_groups', None)
        )
    ...
    def filter(self, X, ops=None, ibatch=None, skip_preproc=False):
        ...
        if self.do_CAR:
            ...
            if self.channel_groups is not None:
                # Perform CAR separately for each channel group
                for group in self.channel_groups:
                    # Compute median across channels in this group
                    group_median = torch.median(X[group, :], 0)[0]
                    # Subtract from only those channels
                    X[group, :] = X[group, :] - group_median
            else:
                # Standard CAR across all channels
...
def save_preprocessing(filename, ops, bfile=None, bfile_path=None, verbose=False):
    ...
    if bfile is None:
        ...
        bfile = BinaryFiltered(
            ...
            dtype=dtype, channel_groups=ops.get('channel_groups', None)
            )
