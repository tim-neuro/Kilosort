...
def run_kilosort(settings, probe=None, probe_name=None, filename=None,
    ...
    channel_groups : list; optional.
        a set of channel groups. e.g. [[0, 1, 2], [3, 4, 5]]. If provided,
        CAR will be applied to each channel group separately. 
        Sorting will still be done on all channels as before. 
...
def _sort(filename, results_dir, probe, settings, data_dtype, device, do_CAR,
          ...
          save_extra_vars, file_object, progress_bar, channel_groups=None,
          gui_sorter=None):
    ...
    try:
        ...
        ops, settings = initialize_ops(
            ...
            device, save_preprocessed_copy, channel_groups,
            gui_mode=(gui_sorter is not None)
            )
...
def initialize_ops(settings, probe, data_dtype, do_CAR, invert_sign,
                   device, save_preprocessed_copy, channel_groups=None,
                   gui_mode=False) -> dict:
    ...
    ops['channel_groups'] = channel_groups
...
def compute_preprocessing(ops, device, tic0=np.nan, file_object=None):
    ...
    bfile = io.BinaryFiltered(ops['filename'], n_chan_bin, fs, NT, nt, twav_min,
                              ...
                              channel_groups=ops['channel_groups'])
